-- SiteScribe MySQL 8.x – Full schema (Prisma ile uyumlu)
-- MySQL Workbench: Bu dosyayı açıp tümünü çalıştır (Execute → Run All).
-- DİKKAT: Aşağıdaki DROP veritabanını ve tüm veriyi siler; sonra sıfırdan oluşturur (uyarı kalmaz).
...
-- ============ Change detection (AI alanı dahil) ============
...
CREATE TABLE IF NOT EXISTS `EventSignal` (
    `id` VARCHAR(191) NOT NULL,
...
-- ============ Change Orders (AI alanları dahil) ============
...
-- ============ CO Template ============
CREATE TABLE IF NOT EXISTS `COTemplate` (
    `id` VARCHAR(191) NOT NULL,
    `organization_id` VARCHAR(191) NOT NULL,
    `name` VARCHAR(191) NOT NULL,
...
-- ============ CO Approval ============
CREATE TABLE IF NOT EXISTS `COApproval` (
    `id` VARCHAR(191) NOT NULL,
    `change_order_id` VARCHAR(191) NOT NULL,
    `user_id` VARCHAR(191) NOT NULL,
