    ...
    <div className="min-h-screen bg-background">
      <header className="page-header sticky top-0 z-30 p-4 sm:px-6 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
        ...
        <div className="flex flex-wrap gap-2 items-center">
          <ThemeToggle />
          <LanguageSwitcher />
          <Link href="/guide" className="nav-link min-h-[44px] flex items-center sm:min-h-0">
            {t('nav.guide')}
          </Link>
          <Link href="/friends" className="nav-link min-h-[44px] flex items-center sm:min-h-0">
            {t('nav.friends')}
          </Link>
          <Link href="/chat" className="nav-link min-h-[44px] flex items-center sm:min-h-0">
            {t('nav.chat')}
          </Link>
          <Link href="/notifications" className="nav-link min-h-[44px] flex items-center sm:min-h-0">
            {t('nav.notifications')}{unreadCount > 0 ? ` (${unreadCount})` : ''}
          </Link>
          <span className="text-sm text-muted-foreground truncate max-w-[180px] sm:max-w-none px-2">{session.user?.email}</span>
          <form action="/api/auth/signout" method="POST">
            <Button type="submit" variant="outline" size="sm" className="min-h-[44px] sm:min-h-0">{t('nav.signOut')}</Button>
          </form>
        </div>
      </header>
